    ...
    def process(self):
        ...
        if self.dataset == 'tdcDDI':
            ...
            for i,data in X.items():
                ...
                # 添加药物ID
                drug_a_id, drug_b_id = drug_ids[i]
                data.drug_a_id = torch.tensor([hash(drug_a_id) % 10000], dtype=torch.long)
                data.drug_b_id = torch.tensor([hash(drug_b_id) % 10000], dtype=torch.long)
        ...
        # 确保expression属性被保存
        if len(data_list) > 0 and hasattr(data_list[0], 'expression'):
            print(f"✓ Expression属性将被保存到collated data中")
